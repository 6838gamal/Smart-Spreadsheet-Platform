"""
Invoice Pipeline — optimised for فاتورة / Invoice documents.
Steps: OCR → Layout → Tables (priority) → NER[invoice schema] → Cleaning → Suggestions
"""
...
class InvoicePipeline(BasePipeline):
    ...
    def _step_layout(self, ctx: PipelineContext):
        ...
        ctx.has_tables   = result.has_tables
        ctx.layout_elements = [
            {
    ...
    def _step_tables(self, ctx: PipelineContext):
        tables = self._table.extract(ctx.file_path)
        ctx.tables = tables
        if tables:
            ctx.has_tables = True

    def _step_ner(self, ctx: PipelineContext):
        entities = self._ner.extract(ctx.raw_text, "invoice")
        ctx.entities = [
            {
    ...
    def _step_cleaning(self, ctx: PipelineContext):
        """Normalise entity values (dates, amounts)."""
        for ent in ctx.entities:
            if ent.get("entity_type") in {"date", "invoice_date", "due_date"}:
                ent["normalized_value"] = ent.get("normalized_value") or ent["value"]
            if ent.get("entity_type") in {"total", "subtotal", "tax_amount", "amount"}:
                # strip non-numeric for normalisation
                raw = ent["value"].replace(",", "").replace(" ", "")
                import re
                nums = re.findall(r"[\d.]+", raw)
                if nums:
                    ent["normalized_value"] = nums[0]
