"""
Generic Pipeline — works for any document type.
Steps: OCR → Layout → Table → NER → Suggestions
"""
...
class GenericPipeline(BasePipeline):
    ...
    def _step_ocr(self, ctx: PipelineContext):
        ...
        # For spreadsheets, raw_text stays empty; tables will be extracted directly
    ...
    def _step_layout(self, ctx: PipelineContext):
        ...
        ctx.has_tables = result.has_tables or ctx.has_tables
        ctx.layout_elements = [
            {
    ...
    def _step_ner(self, ctx: PipelineContext):
        entities = self._ner.extract(ctx.raw_text, ctx.extra.get("doc_type", "unknown"))
        ctx.entities = [
            {
    ...
    def _step_suggestions(self, ctx: PipelineContext):
        suggs = self._suggest.generate(
            doc_type=ctx.extra.get("doc_type", "unknown"),
