"""
Bank Statement Pipeline — optimised for كشف حساب / Bank Statement documents.
Steps: OCR → Layout → Tables (priority) → NER[bank schema] → Suggestions
"""
...
class BankStatementPipeline(BasePipeline):
    name = "bank_statement"
    ...
    def _step_ner(self, ctx: PipelineContext):
        entities = self._ner.extract(ctx.raw_text, "bank_statement")
        ctx.entities = [
            {
