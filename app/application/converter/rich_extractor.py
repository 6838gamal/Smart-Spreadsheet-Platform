...
_LATEX_CMDS    = re.compile(
    r'\\(?:frac|sqrt|sum|int|prod|lim|alpha|beta|gamma|delta|epsilon|'
    r'theta|lambda|mu|pi|sigma|omega|Sigma|Omega|Delta|Gamma|nabla|'
    r'partial|infty|pm|times|div|cdot|leq|geq|neq|approx|equiv|'
    r'sin|cos|tan|log|ln|exp|vec|hat|bar|dot|ddot|left|right|'
    r'begin|end|matrix|bmatrix|pmatrix|cases|text|mathbb|mathbf|'
    r'mathrm|mathcal|overline|underline|overbrace|underbrace)'
)

# Chemical formula pattern: sequences like H2O, CO2, C6H12O6, Fe2O3 etc.
_CHEM_FORMULA  = re.compile(r'\b([A-Z][a-z]?\d*)+\b')
